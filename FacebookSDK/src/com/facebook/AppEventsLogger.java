...
public class AppEventsLogger {
	...
	private AppEventsLogger(Context context, String applicationId,
		...
		// session's app ID:
		if (session != null
				&& (applicationId == null || applicationId.equals(session
						.getApplicationId()))) {
	...
	private static SessionEventsState getSessionEventsState(Context context,
		...
		// blocks waiting on the main thread,
	...
	private static void setSourceApplication(Activity activity) {
		...
		// was opened by an intent before.
		// Introduce an extra field in the intent to force clear the
		// sourceApplication.
	...
	static class SessionEventsState {
		...
		private void populateRequest(Request request, int numSkipped,
			...
			// The code to get all the Extended info is safe but just in case we
			// can wrap the whole
			// call in its own try/catch block since some of the things it does
			// might cause
			// unexpected exceptions on rooted/funky devices:
			try {
				Utility.setAppEventExtendedDeviceInfoParameters(publishParams,
						applicationContext);
			} catch (Exception e) {
				// Swallow
			}
