...
public class VCardProvider {
	...
	private static class VCardReader {
		...
		public void initializeFields() {
			...
			vCard.setField("SORT-STRING", getTagContents("SORT-STRING"));
			vCard.setJabberId(getTagContents("JABBERID"));
			setupSimpleFields();
			setupPhones();
			setupAddresses();
		}
		...
		private void setupAddresses() {
			...
			for (int i = 0; i < allAddresses.getLength(); i++) {
				...
				for (int j = 0; j < value.size(); j++) {
					if ("HOME".equals(type)) {
						vCard.setAddressFieldHome((String) code.get(j),
								(String) value.get(j));
					} else { // By default, setup work address
						vCard.setAddressFieldWork((String) code.get(j),
								(String) value.get(j));
					}
				}
			}
