...
package com.wifin.kachingme.security.jncryptor;
...
public class AES256JNCryptor implements JNCryptor {
	...
	public SecretKey keyForPassword(char[] password, byte[] salt)
		...
		Validate.isTrue(salt.length == SALT_LENGTH,
				"Salt value must be %d bytes.", SALT_LENGTH);
	...
	private byte[] decryptV2Data(AES256v2Ciphertext aesCiphertext,
		...
		try {
			...
			cipher.init(Cipher.DECRYPT_MODE, decryptionKey,
					new IvParameterSpec(aesCiphertext.getIv()));
	...
	private byte[] decryptV2Data(byte[] ciphertext, char[] password)
			throws CryptorException {
		try {
			AES256v2Ciphertext aesCiphertext = new AES256v2Ciphertext(
					ciphertext);
			...
			SecretKey hmacKey = keyForPassword(password,
					aesCiphertext.getHmacSalt());
	...
	private byte[] decryptV3Data(byte[] ciphertext, char[] password)
			throws CryptorException {
		try {
			AES256v3Ciphertext aesCiphertext = new AES256v3Ciphertext(
					ciphertext);
	...
	public byte[] encryptData(byte[] plaintext, char[] password,
			byte[] encryptionSalt, byte[] hmacSalt, byte[] iv)
			throws CryptorException {
		...
		return encryptData(plaintext, new PasswordKey(encryptionKey,
				encryptionSalt), new PasswordKey(hmacKey, hmacSalt), iv);
	}
	...
	public byte[] decryptData(byte[] ciphertext, SecretKey decryptionKey,
		...
		try {
			...
			switch (version) {
			case 2:
				return decryptV2Data(new AES256v2Ciphertext(ciphertext),
						decryptionKey, hmacKey);
			case 3:
				return decryptV3Data(new AES256v3Ciphertext(ciphertext),
						decryptionKey, hmacKey);
			default:
				throw new CryptorException(String.format(
						"Unrecognised version number: %d.", version));
	...
	byte[] encryptData(byte[] plaintext, PasswordKey encryptionKey,
			PasswordKey hmacKey, byte[] iv) throws CryptorException {
		try {
			Cipher cipher = Cipher.getInstance(AES_CIPHER_ALGORITHM);
			...
			AES256v3Ciphertext output = new AES256v3Ciphertext(
					encryptionKey.getSalt(), hmacKey.getSalt(), iv, ciphertext);
